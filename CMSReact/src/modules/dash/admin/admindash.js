...
const AdminDashComponent = () => {
	...
	const reloadAppointments = (companyid, startDate, endDate, userId, timezoneText) => {
		...
		if(userId === undefined || userId === ""){
			axiosFunction = AppointmentService.fetchQuickAppointments
		}else{
			axiosFunction = AppointmentService.fetchDoctorAppointments
		}
		let formObject = {
			companyId: companyid,
			startDate: startDate,
		...
		axiosFunction(formObject).then((res) => {
			let resArr = res.data;
			console.log(resArr + "Fetch appointments");
