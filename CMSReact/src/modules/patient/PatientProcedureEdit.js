...
const PatientProcedureEdit = (props) => {
    ...
    if (isMainDataLoading) {
      PatientService.fetchPatientTreatmentByTreatmentId(idParam)
        .then((response) => {
          ...
          console.log(treatmentInfo[0].treatmentProcedures)
          let procedureList = [];
          //Loop through the procedures list and format to ui objects
          treatmentInfo[0].treatmentProcedures.map((proc) => {
            const procedure = {
              ...PatientConstants.patientTreatmentProcedureObject,
            };
            console.log("in pat proc edit")
          ...
          //Get logged in user name ( need to fix)
  ...
  const submitForm = (e) => {
    ...
    //Format the date
    console.log("patient proc submitstate ")
    console.log(submitState[0])
    ...
    submitState[0].treatmentProcedures = procedures;

    PatientService.updatePatientTreatment(submitState[0])
      .then((response) => {
        dispatchPageState({
          field: "openSnackMessage",
          value: true,
