...
const PatientProcedureAdd = (props) => {
  ...
  const handleClose = (event, reason) => {
    ...
    if (submitType === "INV") {
      console.log("patientIdParam: " + patientIdParam + " savedTreatmentId:" + savedTreatmentId)
      history.push(
  ...
  const submitForm = (e) => {
    ...
    submitState.id = Math.floor(Math.random() * 10000000) + 1; 
    submitState.treatmentDate = moment(
      submitState.treatmentDate,
      "YYYY-MM-DD HH:mm"
    );
    //const treatId = Math.floor(Math.random() * 10000000) + 1;  
    ...
    procedures.map((proc, index) => {
      ...
      procedure.treatmentId =  submitState.id;
    ...
    PatientService.addPatientTreatment(submitState)
      .then((response) => {
        if (response.status === 200) {
          console.log("Helper.getNumberFromString(response.data.procedureId)" )
