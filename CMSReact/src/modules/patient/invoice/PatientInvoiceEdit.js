...
const PatientInvoiceEdit = (props) => {
  ...
  const getItemList = () => {
    MasterService.fetchAllItemsByCompanyId(
      ...
      .then((response) => {
        console.log("getitemList in invoice edit")
    ...
    if (isMainDataLoading) {
      PatientService.fetchPatientInvoiceByInvoiceId(idParam)
        .then((response) => {
          ...
          console.log("invoiceInfo.invoiceDetails")
          console.log(invoiceInfo)
          invoiceInfo.invoiceDetails.map((proc) => {
            ...
            if (procedure.detailType === "I") {
              MasterService.fetchItemByItemId(proc.itemId)
              .then((res) => {
              procedure.stockAvailability = res.data.avaialableStock;
              procedure.reorderLevel = res.data.reorderLevel;
              procedure.stockingUnit = res.data.stockingUnit;
              })
            }
          ...
          console.log("invoice details new")
