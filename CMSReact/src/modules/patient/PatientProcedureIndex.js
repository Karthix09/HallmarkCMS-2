...
export default function PatientProcedureIndex(props) {
  ...
  const getPatientProceduresByPatientId = (id) => {
    PatientService.fetchPatientProceduresByPatientId(id)
      .then((response) => {
        console.log("in pat proc index")
        console.log(response.data)
        const resultArray = Array.isArray(response.data) ? response.data : []; 
        resultArray.map((resArr) => {
            resArr.treatmentProcedures.map((treatProc , index ) => {
              // code to get the Procedure Name and append
              PatientService.fetchProcedureNameById(treatProc.procedureId)
              .then((resp) => {
                    console.log(treatProc.procedureId + ":"+ resp.data.procedureName) 
                    treatProc["procedureName"] = resp.data.procedureName
                    console.log("treatArray")
                    console.log(treatProc)
              })
            })
        })
