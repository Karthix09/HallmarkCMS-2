...
class PatientService {
	// fetchPatients1 = (searchTerms) => {
	// 	return axios.get(`${BACKEND_API_URL}/patient/search`, searchTerms);
	// };
	...
	fetchPatientByName = (patientName) => {
		return axios.get(`${BACKEND_API_URL}/patient/patientName/${patientName}`);
	};

	fetchAppointmentsByPatientId = (searchTerms) => {
		return axios.get(`${BACKEND_API_URL}/appointment/byPatient/${searchTerms.patientId}`, searchTerms);
	};
	...
	// updatePatientClinicalNote = (id) => {
	// 	return axios.post(`${BACKEND_API_URL}/patient/notes/updateNote/${id}`);
	// };

	updatePatientClinicalNote = (notesData) => {
			return axios.post(`${BACKEND_API_URL}/patient/notes/updateNote`,notesData);
		};
	...
	searchPatientName = (patientName) => {
		return axios.get(`${BACKEND_API_URL}/patient/${patientName}`);
	};
	...
	getmcReport =(id)=>{
		return axios.get(`${BACKEND_API_URL}/patient/getmcReport/${id}`, AuthService.getAuthHeader());
	}

	fetchProcedureNameById = (id) => {
		return axios.get(`${BACKEND_API_URL}/procedure/name/${id}`, AuthService.getAuthHeader());
	};
}
